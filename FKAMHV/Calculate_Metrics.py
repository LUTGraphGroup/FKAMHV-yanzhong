...
class Metric_fun(object):
    ...
    def cv_mat_model_evaluate1(self, association_mat, predict_mat):
    ...
    def get_metrics(self, real_score, predict_score):
        ...
        #print("Precision List:", precision_list.shape)
    ...
    def get_metrics1(self, real_score, predict_score):
        sorted_predict_score = np.array(sorted(list(set(np.array(predict_score).flatten()))))
        sorted_predict_score_num = len(sorted_predict_score)
        ...
        return [auc[0, 0], fpr, tpr]  # 返回 AUC、FPR 和 TPR
